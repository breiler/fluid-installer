...
const InstallerModal = ({ state, progress, errorMessage, onClose }: InstallerModalProps) => {
    ...
    return (
        <Modal centered show={true}>
            <Modal.Body>
                {(state === InstallerState.DOWNLOADING || state === InstallerState.CHECKING_SIGNATURES) && (<><PageTitle>Downloading</PageTitle><p>Downloading package... <Spinner /></p></>)}
                {state === InstallerState.ENTER_FLASH_MODE && (<BootloaderInfo/>)}
                {state === InstallerState.FLASHING && (<Progress progress={progress} status={state} />)}
                {state === InstallerState.RESTARTING && (<><PageTitle>Restarting</PageTitle><p>Waiting for controller restart... <Spinner /></p></>)}
