...
export const fetchAsset = async (asset) => {
  return fetch(CORS_PROXY_URL + asset.browser_download_url).then((response) => {
 ...
 * @returns
 */
export const unzipAssetData = (zipData, firmwareType = FirmwareType.WIFI) => {
  return JSZip.loadAsync(zipData).then((zip) => {
    const bootLoaderFile = findFileInZip(zip, "/common/bootloader_dio_80m.bin");
